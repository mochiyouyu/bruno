...
const RunnerTags = ({ collectionUid, className = '' }) => {
  ...
  const handleValidation = (tag) => {
    ...
    if (!availableTags.includes(trimmedTag)) {
      return t('RUNNER.TAGS.ERRORS.NOT_EXISTS', { defaultValue: 'tag does not exist!' });
    }
    if (tags.include.includes(trimmedTag)) {
      return t('RUNNER.TAGS.ERRORS.ALREADY_IN_INCLUDE', { defaultValue: 'tag already present in the include list!' });
    }
    if (tags.exclude.includes(trimmedTag)) {
      return t('RUNNER.TAGS.ERRORS.ALREADY_IN_EXCLUDE', { defaultValue: 'tag is present in the exclude list!' });
    }
  };
    ...
    <div className={`flex flex-col ${className}`}>
      <div className="flex flex-row gap-4 w-full">
        <div className="flex-1 flex flex-col gap-2 min-w-0">
          <span>{t('RUNNER.TAGS.INCLUDE', { defaultValue: 'Include tags' })}</span>
          <TagList
            tags={tags.include}
        ...
        <div className="flex-1 flex flex-col gap-2 min-w-0">
          <span>{t('RUNNER.TAGS.EXCLUDE', { defaultValue: 'Exclude tags' })}</span>
          <TagList
            tags={tags.exclude}
